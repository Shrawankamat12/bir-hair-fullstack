 ...
 *
 * "online" = Bluevine Payment Link. After placing the order, the
 * customer is redirected to a fixed Bluevine payment page to pay
 * manually. There is no embedded checkout widget anymore — the order
 * is created with payment status "pending" and stays that way until
 * an admin manually marks it Paid once the payment is confirmed.
 *
 * COD is handled by our store and needs no online payment step.
 */
const PAYMENT_OPTIONS = [
  {
    id: 'online',
    title: 'Pay Online (Bluevine)',
    sub: 'Pay securely via our Bluevine Payment Link',
    icon: FiCreditCard,
  },
  {
    id: 'cod',
...
export default function Checkout() {
  ...
  /*
   * Create order. For "online" payment, redirect the customer to the
   * Bluevine Payment Link afterwards — order stays pending/unpaid until
   * an admin confirms the payment manually.
   */
    ...
    try {
      ...
      const res = await ordersApi.create({
        ...
        items: cart.map((item) => {
          ...
          // Per-unit discount — do NOT multiply by quantity here.
          // The backend (order.service.js) computes finalPrice as
        ...
        // Validator (order.validator.js) reads these as flat top-level
        // fields, not nested under payment/shipping — sending both keeps
        // it working regardless of which one the backend actually reads.
      ...
      /*
       * COD — no online payment step needed.
       */
      if (payMethod === 'cod') {
        clearCart();
      ...
      /*
       * ONLINE PAYMENT (Bluevine Payment Link)
       *
       * Fetch the configured payment link from the backend and send the
       * customer to the order-confirmation page, which shows a "Pay Now"
       * button/instructions pointing at that link. The order remains
       * "Pending / Unpaid" until an admin marks it Paid manually.
       */
  ...
  function next() {
    ...
    if (step === 0 && !addressValid()) {
      setFormError(
        'Please fill in name, phone, address, city, state and pincode.'
      );
                        ...
                        {payMethod !== 'cod' && (
                          <div className="mt-4 flex items-start gap-2 rounded-xl bg-[#fff5f8] px-3 py-3 text-xs text-gray-500">
                            ...
                            <span>
                              ...
                              you'll be taken to our secure
                              Bluevine Payment Link to complete
                              your payment.
                              <br />
                              Your order will show as{' '}
                              <strong>Pending / Unpaid</strong>{' '}
                              until we confirm your payment.
                            </span>
