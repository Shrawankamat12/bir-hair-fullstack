...
export default function OrderConfirmation() {
  ...
  // Present only when the customer chose "Pay Online (Bluevine)" at checkout.
  const paymentMethod = location.state?.paymentMethod;
  const paymentLink = location.state?.paymentLink;
  const amount = location.state?.amount;
  const isOnlinePayment = paymentMethod === 'online' && Boolean(paymentLink);
        ...
        {isOnlinePayment && (
          ...
          >
            ...
            <p style={{ marginTop: '0.5rem', marginBottom: '0.75rem' }}>
              Your order is currently <strong>Pending / Unpaid</strong>.
              Please pay{amount ? ` ₹${Number(amount).toLocaleString('en-IN')}` : ''} using
              the secure Bluevine Payment Link below. Once we confirm your
              payment, we'll update your order status.
            </p>
            <a
              href={paymentLink}
              target="_blank"
            ...
            >
              Pay Now via Bluevine <FiExternalLink size={14} />
            </a>
            <p style={{ marginTop: '0.75rem', fontSize: '0.8rem', opacity: 0.75 }}>
              Please mention your Order ID (<strong>{orderId}</strong>) as a
              reference/note if the payment page allows it, so we can match
