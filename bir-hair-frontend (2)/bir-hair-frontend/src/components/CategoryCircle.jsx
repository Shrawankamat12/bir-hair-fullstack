...
export default function CategoryCircle({ cat, variant = 'circle' }) {
  ...
  if (variant === 'card') {
    return (
      <Link to={`/shop?category=${cat.slug}`} className={`catcard tone-${cat.tone || 'gold'}`}>
        <div
          ref={ref}
          className="catcard-media"
          onMouseMove={onMove}
          onMouseLeave={onLeave}
          style={imageUrl ? { backgroundImage: `url(${imageUrl})` } : undefined}
        >
          {!imageUrl && <span className="catcard-glyph">{cat.name.charAt(0)}</span>}
        </div>
        <span className="catcard-label">{cat.name}</span>
      </Link>
    );
  }
